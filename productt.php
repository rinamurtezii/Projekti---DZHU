...
<div class="product-page">
    <div class="product-layout">
        ...
        <div class="product-info">
            ...
            <p class="subtitle"><?= htmlspecialchars($product['description']) ?></p>

            <div class="rating">★★★★★ (5.0)</div>

            <hr>

            <p class="product-p"><?= number_format($product['price'], 2) ?>€</p>
            ...
            <button class="buy-btn add-to-cart-btn"
                ...
                data-img="<?= htmlspecialchars($product['image']) ?>">
                Add to Cart
            </button>
        </div>
    </div>
...
<!-- KEY FEATURES (STATIC, si në HTML-in tënd) -->
<div class="keyfeatures">
    <h2>Key Features</h2>
    <br>
    <ul>
        <?php
        // Split features CSV në array dhe printo secilin si <li>
        if (!empty($product['features'])) {
            $features = explode(',', $product['features']);
            foreach ($features as $feature) {
                echo '<li><img src="check.png" width="20" height="20"> ' . htmlspecialchars(trim($feature)) . '</li>';
            }
        }
        ?>
    </ul>
